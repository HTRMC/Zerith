...
Player::Player(const glm::vec3& position) 
    : m_position(position), m_velocity(0.0f), m_rotation(0.0f) {
    updateAABB();
...
void Player::handleInput(GLFWwindow* window, float deltaTime) {
    ...
    if (glm::length(movement) > 0.0f) {
        ...
        float cosYaw = cos(m_rotation.y);
        float sinYaw = sin(m_rotation.y);
        
        glm::vec3 forward(-sinYaw, 0.0f, -cosYaw);
        glm::vec3 right(-cosYaw, 0.0f, sinYaw);
        
        glm::vec3 worldMovement = forward * movement.z + right * movement.x;
        
        m_velocity.x = worldMovement.x * MOVE_SPEED;
        m_velocity.z = worldMovement.z * MOVE_SPEED;
    } else {
        m_velocity.x = 0.0f;
        m_velocity.z = 0.0f;
    }
